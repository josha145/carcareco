...
export interface IObjectToNode<T> {
  (item: T): ReactNode
}
...
export default function TypeAheadCombobox<T>({
  ...
  optionFormatter: IObjectToNode<T>,
  placeholder: string,
  className?: string,
      ...
      <div className={comboboxOptionsAbsolute ? '' : 'mt-2 relative'}>
        <div className="grid grid-cols-1">
          ...
          {clearable && (
            <XMarkIcon
              aria-hidden="true"
              onClick={() => {}}
              className="cursor-pointer col-start-1 row-start-1 mr-2 self-center justify-self-end text-gray-500 size-4"
            />
          )}
        </div>
        ...
        {datasource.length > 0 && (
          <ComboboxOptions
            modal={false}
            className={clsx(
              comboboxOptionsAbsolute ? ("w-80 sm:w-" + comboboxOptionsWidth) : "w-full",
              "absolute z-10 mt-1 max-h-60 overflow-auto rounded-md bg-white py-1 text-base ring-1 shadow-lg ring-black/5 focus:outline-hidden text-sm"
            )}
          >
